#!/usr/bin/env python3
"""Run the pre-candidate twelve-process Vivado stability gate for v2."""
...
def command(vivado: Path, result: Path, reports: Path) -> list[str]:
    return [
        str(vivado), "-mode", "batch", "-nojournal", "-nolog", "-notrace",
        "-source", str(TCL), "-tclargs", str(PROBE),
        "timing_gate_v2_stability_probe", "clk", "10.0", str(result),
        str(reports), "xc7z020clg400-1",
    ]


def internal_tcl_error(stdout: str, stderr: str) -> bool:
    text = stdout + "\n" + stderr
    return ("couldn't read file \"C:/Xilinx/Vivado/2023.1/scripts/rt/data/" in text
            or "cannot open file \"C:/Xilinx/Vivado/2023.1/scripts/rt/data/" in text)


def run_one(index: int, vivado: Path, baseline: dict, campaign: Path) -> dict:
    ...
    completed = None
    if before_ok:
        cmd = command(vivado, result, reports)
        with stdout_path.open("w", encoding="utf-8", errors="replace") as stdout, \
            ...
            completed = subprocess.run(
                subprocess.list2cmdline(cmd), cwd=trial, stdout=stdout,
    ...
    elif internal_tcl_error(stdout, stderr):
...
def build_attestation(baseline: dict, rows: list[dict]) -> dict:
    ...
    try:
        ...
        for index in range(1, RUNS + 1):
            ...
            if not row["valid_measurement"]:
                break
        attestation = build_attestation(baseline, rows)
