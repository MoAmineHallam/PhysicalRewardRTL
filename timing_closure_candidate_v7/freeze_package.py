#!/usr/bin/env python3
"""Freeze or verify the pre-outcome candidate-V7 package by raw bytes."""
...
try:
    from .common import (
        HERE, ROOT, PACKAGE_MANIFEST, PACKAGE_SHA, atomic_json,
...
ROOT_FILES = ("launch_timing_candidates_v7.ps1",)
...
def dependency_paths() -> list[Path]:
    ...
    paths.extend([v1 / "manifest.json", v1 / "manifest.sha256",
                  v1 / "closure_synth.tcl", v1 / "run_closure.py",
                  v1 / "validate_gate.py"])
    paths.extend(sorted((v1 / "inputs").glob("*.sv")))
    v7 = ROOT / "timing_closure_gate_v7"
    paths.extend([
        v7 / "manifest.json", v7 / "manifest.sha256",
        v7 / "dependency_baseline.json", v7 / "stability_common.py",
        v7 / "stability_campaign_001" / "stability_attestation.json",
        v7 / "stability_campaign_001" / "stability_attestation.sha256",
    ])
    v3 = ROOT / "timing_closure_candidate_v3"
    paths.extend([v3 / "manifest.json", v3 / "manifest.sha256",
                  v3 / "results" / "pilot_gate.json"])
    return paths
...
def build_manifest() -> dict:
    ...
    return {
        ...
        "status": "frozen before first Vivado-2026.1 candidate process",
...
def main() -> int:
    ...
    try:
        ...
        if args.write:
            if (HERE / "results").exists() or Path(r"C:\VGC7P001").exists():
                raise RuntimeError(
                    "candidate results/scratch exist; pre-outcome freeze refused")
