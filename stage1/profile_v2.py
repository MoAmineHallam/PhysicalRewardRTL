...
def run(a):
    ...
    model.set_decode_backend(a.decode_backend)
            ...
            for mode in ['prefill', 'decode']:
                ...
                row = {'length': length, 'batch': 1, 'cache': kind, 'mode': mode,
                       ...
                       'logical_not_dram_bytes': {
                           'ffn_weights': sum(p.numel()*p.element_size()
                                              for block in model.blocks for p in block.ffn.parameters()),
...
def main():
    ...
    p.add_argument('--decode-backend', choices=['sdpa', 'explicit'], default='sdpa')
