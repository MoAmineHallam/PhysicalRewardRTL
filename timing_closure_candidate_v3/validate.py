#!/usr/bin/env python3
"""Validate V3 candidate records and publish the frozen pilot verdict."""
...
try:
    from .common import (V3_ATTESTATION, V3_ATTESTATION_SHA, atomic_json,
                         canonical_json_bytes, read_json, sha256_file,
...
def validate_trial(row: dict, candidate: dict, v1_digest: str,
    ...
    if row.get("valid_measurement"):
        ...
        expected_closed = bool(int(vivado.get("closed", 0)) == 1)
        if row.get("closed") is not expected_closed:
...
def build_gate(manifest: dict, v1_digest: str, package_digest: str,
    ...
    generated_from = [
        ...
        {"path": "timing_closure_gate_v3/stability_campaign_001/stability_attestation.json",
         "sha256_raw": V3_ATTESTATION_SHA},
        {"path": "timing_closure_candidate_v3/manifest.json",
         "sha256_raw": package_digest},
    ]
    for candidate in manifest["candidates"]:
        result, path, errors = load_result(candidate, v1_digest,
                                           package_digest, results)
        if result is None:
    ...
    complete = (not failures
                and all(result.get("status") == "COMPLETE"
    ...
    criteria = {
        ...
        "paired_direction": bool(paired_mean is not None and paired_mean > 0.0
                                  and reversals <= 1),
    }
    ...
    return {
        ...
        "v1_manifest_sha256": v1_digest,
        "candidate_package_sha256": package_digest,
        "v3_stability_attestation_sha256": V3_ATTESTATION_SHA,
...
def main() -> int:
    ...
    try:
        ...
        if args.scope == "pilot":
            gate, failures = build_gate(manifest, v1_digest, package_digest,
                                        Path(args.results))
            if gate is None:
            ...
            if output.exists():
                ...
                atomic_json(output, gate, replace=False)
