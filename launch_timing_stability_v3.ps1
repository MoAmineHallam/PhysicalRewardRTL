...
$gate = Join-Path $repo "timing_closure_gate_v3"
...
git diff --quiet -- .gitattributes launch_timing_stability_v3.ps1 timing_closure_gate_v3
if ($LASTEXITCODE -ne 0) {
    throw "the frozen v3 launcher/package has unstaged changes"
}
git diff --cached --quiet -- .gitattributes launch_timing_stability_v3.ps1 timing_closure_gate_v3
if ($LASTEXITCODE -ne 0) {
    throw "the frozen v3 launcher/package has staged changes"
}
if (Test-Path -LiteralPath $campaign) {
    throw "v3 stability campaign already exists and cannot be overwritten: $campaign"
}
if (Test-Path -LiteralPath $scratch) {
    throw "v3 scratch root already exists and cannot be overwritten: $scratch"
}
if (Get-Process -Name vivado -ErrorAction SilentlyContinue) {
    throw "another Vivado process is already running"
}
Invoke-LoggedPython -PythonArgs @("-m", "unittest", "timing_closure_gate_v3.test_stability")
