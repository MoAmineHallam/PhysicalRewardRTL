...
\item \textbf{A cheap repair, and evidence that offline metrics cannot see it.}
Forty-three real measurements from the optimized region reduce error to
$+30.7$\,MHz and improve every downstream outcome. Under a matched-set
leave-one-design-out comparison---identical target rows, normalisation refit
within each fold---the repaired and original predictors are
\emph{indistinguishable} ($\rho = 0.919$ against $0.930$, with per-design top-1
favouring the repaired one). No offline metric available at selection time
separates two rewards whose behaviour under optimization differs by an order of
magnitude (Section~\ref{sec:repair}).
\end{enumerate}

% fig1 appears automatically once make_figs.py can build it
\IfFileExists{figures/fig1_trajectory.tex}{\input{figures/fig1_trajectory}}{}

% ---------------------------------------------------------------------------
% Method diagram. Hand-written TikZ so it stays editable in Overleaf.
% ---------------------------------------------------------------------------
\begin{figure}[t]
\centering
\begin{tikzpicture}[
  ...
  cheap/.style = {box, fill=blue!6},
  study/.style = {box, fill=red!10, very thick},
  truth/.style = {box, fill=black!5},
  ar/.style    = {-{Latex[length=1.6mm]}, thin}
]
\node[box] (pi) {policy $\pi_\theta$};
\node[box, right=7mm of pi] (samp) {$G{=}8$\\samples};
\node[cheap, right=7mm of samp] (orc) {oracle\\$\sim$1\,s};
\node[study, right=7mm of orc] (sur) {\textbf{surrogate} $\hat{F}$\\$\sim$1\,ms};
\node[box, below=6mm of sur] (adv) {group-relative\\advantage};
\node[box, left=7mm of adv] (upd) {update $\theta$};

\draw[ar] (pi) -- (samp);
\draw[ar] (samp) -- node[above]{RTL} (orc);
\draw[ar] (orc) -- node[above]{correct} (sur);
\draw[ar] (sur) -- (adv);
\draw[ar] (adv) -- (upd);
\draw[ar] (upd) -| (pi);
\draw[ar] (orc.south) |- ++(0,-4mm) -| node[pos=.25,below]{incorrect: $r{=}0$} (adv.west);

% ground truth, deliberately outside the loop
\node[truth, below=13mm of upd, xshift=6mm] (viv)
  {Vivado implementation\\$\sim$1\,min \emph{per design}};
\draw[ar, dashed] (viv.north) -- node[right, pos=.45]{too slow for the loop} (upd.south);
\node[anchor=west, align=left, font=\scriptsize\itshape, gray]
  at ([xshift=3mm]viv.east) {ground truth\\(this paper)};

\begin{scope}[on background layer]
  \node[draw, dashed, gray, rounded corners, fit=(pi)(samp)(orc)(sur)(adv)(upd),
        inner sep=3mm, label={[gray,font=\scriptsize]above right:training loop}] {};
\end{scope}
\end{tikzpicture}
\caption{Where the learned reward sits. The oracle and the surrogate are both
cheap enough to query for every sample of every group; the synthesis tool is
not, which is why the surrogate exists. The surrogate is fitted on data from the
supervised policy and then queried, throughout training, on a distribution that
optimization is actively moving away from it. This paper measures the
consequence against the tool in the lower box.}
\label{fig:pipeline}
\end{figure}
...
\IfFileExists{figures/fig3_scatter.tex}{\input{figures/fig3_scatter}}{}

Three observations follow, and Fig.~\ref{fig:scatter} shows their geometry: the
supervised cells lie along the identity line, while the optimized cells lift off
it and accumulate against the clip bound.
...
\begin{table}[t]
\caption{Predictor output, measured frequency and functional correctness along
the optimization path. Measured \fmax{} is count-weighted over correct
candidates, so it and the prediction describe the same population.}
\label{tab:trajectory}
\centering
\small
\begin{tabular}{lrrrr}
\toprule
checkpoint & updates & correct. & predicted & measured \\
\midrule
supervised & 0   & 94.3\% &  82.9 & 81.0 \\
c1         & 77  & 95.8\% & 218.1 & 240.5 \\
c2         & 138 & 95.3\% & 213.7 & 242.2 \\
c3         & 205 & 95.3\% & 216.6 & 242.2 \\
\textbf{c4} & \textbf{276} & \textbf{88.0\%} & \textbf{497.9} & 241.2 \\
\bottomrule
\end{tabular}
\end{table}
...
Measurement does not accompany it. Real frequency reaches $240$\,MHz by the
first checkpoint and stays within $2$\,MHz of that for the rest of the run,
including across the jump.

Correctness collapses over the same interval, from $95.3\%$ to $88.0\%$, having
been stable or improving until then.

The two facts together identify what the saturated reward destroys. Conditional
on being correct, the policy's output is no faster at the end of training than
it was two hundred updates earlier; what changes after saturation is that it is
correct less often. Scored at equal sample cost---counting an incorrect sample
as $0$\,MHz---the same trajectory reads $230.9 \rightarrow 212.3$\,MHz over the
final interval, and every megahertz of that decline is the correctness term. A
reward that can no longer distinguish one fast design from another does not
make designs slower; it stops penalising the ones that do not work.
...
\midrule
\multicolumn{5}{l}{\footnotesize\emph{offline} matched-set LODO $\rho$:
\;0.930 $\rightarrow$ 0.919 (indistinguishable, Tab.~\ref{tab:lodo})} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{And the conventional offline metric cannot tell the two apart.} We
compare them under leave-one-design-out cross-validation on an
\emph{identical} set of target rows---the repaired predictor's training set is a
strict superset of the original's, so both arms predict the same $229$ shared
rows---with the feature normalisation refit inside every fold.\footnote{Fitting
normalisation over all rows, as our own trainer did until we audited it, leaks
the held-out design's feature distribution into its own evaluation, and leaks it
hardest for designs unlike the rest: exactly the extreme-frequency rows whose
ordering the reward depends on.} Table~\ref{tab:lodo} gives the result.

\begin{table}[t]
\caption{Matched-set leave-one-design-out comparison. Identical target rows,
fold-local normalisation, five restarts averaged per fold. Errors are in MHz
after applying the reward's own clamp.}
\label{tab:lodo}
\centering
\small
\begin{tabular}{lrrrr}
\toprule
predictor & $\rho$ & top-1 & mean err & MAE \\
\midrule
original    & \textbf{0.930} & 25/30 & $+7.0$ & \textbf{20.2} \\
re-anchored & 0.919 & \textbf{26/30} & $\mathbf{+4.5}$ & 21.7 \\
\bottomrule
\end{tabular}
\end{table}

The separation is $0.011$ in rank correlation, and the metrics disagree about
its direction: rank correlation and mean absolute error marginally favour the
original, per-design top-1 accuracy and bias favour the re-anchored one. By any
reading, the two are indistinguishable offline. Under optimization they are not
remotely comparable---bias $+235.9$ against $+30.7$\,MHz, saturated cells $25$
of $30$ against $2$.

We draw the weaker and, we think, more useful conclusion. It is not that
cross-validation prefers the broken reward; on a controlled comparison it does
not prefer either. It is that \emph{no offline metric available at selection
time distinguishes them at all}, while the choice between them determines
whether optimization succeeds or destroys correctness. Offline validity does not
transfer to optimization validity, and the failure is invisible rather than
merely misleading. An earlier draft of this work reported a large offline gap
($0.965 \rightarrow 0.841$) in support of the stronger claim; that comparison
used two different predictor versions scored on two different row sets, and does
not survive being controlled.

We note one incidental result from the same run. Holding out \texttt{med3}---a
design with no near neighbour among the training rows---makes the predictor
extrapolate to a log-frequency of $3.5 \times 10^6$, an absurdity that plain
cross-validation absorbs without comment because rank statistics are insensitive
to it. Both predictors do this. It is the same off-support blow-up the reward
clamp exists to contain, visible here in an offline metric that nonetheless
reports $\rho > 0.9$.
...
\textbf{A defect the audit found in the oracle itself.} The comparison above
tests the acceptance \emph{rule}; it does not test the code that produces the
trace the rule consumes. Inspecting that code, we found that the trace parser
retained only decimal tokens, so a cycle on which the design under test drove an
unknown or high-impedance value---rendered \texttt{x} or \texttt{z} by the
simulator---was silently \emph{deleted} rather than compared. Deletion shortens
the trace and shifts every later sample one position earlier, and because the
rule searches over a latency offset, the shift can be absorbed. The failure mode
is not hypothetical: a design we constructed to be correct except that its final
sixteen output cycles are undefined scores a \emph{perfect} $1.0000$ under the
deleting parser and is accepted.

We now retain such a sample in place as a sentinel that cannot equal any
reference value, so an undefined output counts as a mismatch. Retaining rather
than rejecting outright is deliberate: an unreset pipeline is legitimately
undefined while it fills, and the warmup and latency window already excuses that
prefix. On the same constructed design the corrected parser scores $0.9622$ and
rejects.

Re-auditing all $1{,}529$ candidates under the corrected parser, $18$ emit an
undefined value on at least one cycle, and in every case those cycles fall
entirely within the pipeline-fill prefix the rule already skips; all $18$ still
score exactly $1.0000$. \textbf{No verdict changed.} We report the defect
because a reader is entitled to know that the acceptance criterion was audited
by execution rather than by inspection alone, and because the result---a real
defect that provably could have admitted a wrong design, and provably did
not---is the only kind of statement this audit can honestly make.
...
\textbf{Tool-in-the-loop optimization.} A 2026 line of work optimizes RTL by
calling the synthesis tool itself inside the search rather than learning a
model of it: Dr.\ RTL~\cite{drrtl2026} performs agentic critical-path analysis
and rewriting against an industrial flow, and POET~\cite{poet2026} runs an
LLM-driven evolutionary search with non-dominated sorting over measured PPA.
These methods do not have the failure we describe, by construction---their
signal cannot drift from ground truth because it \emph{is} ground truth. They
pay for that in tool invocations, which is precisely the cost a learned reward
exists to avoid. Our result should be read as the price of that trade: the
proxy is cheap until optimization leaves the region its labels cover, and
nothing in the offline metrics announces when that happens.

\textbf{Learned pre-synthesis PPA prediction.} Predicting post-synthesis
quality from RTL is an established problem with its own
literature~\cite{fang2024presynthesis,ustun2020accurate}, evaluated in the
standard way: held-out accuracy and rank correlation over a fixed corpus. Our
argument is not that these models are inaccurate---ours is accurate by exactly
those metrics, and remains so after it has become useless as a reward. It is
that the metric is measured on a distribution the predictor's own deployment
will not preserve. Any predictor used as an optimization target, not merely as
an estimator, inherits this.
...
in Section~\ref{sec:integrity} is what supports the acceptance rule. We note
that this line of work also raises the bar on evaluation realism---Dr.\
RTL~\cite{drrtl2026} in particular argues for industrial flows over degraded
designs and open-source tools---and that our WNS-derived estimates
(Section~\ref{sec:limitations}) do not meet that bar. Our comparisons are
internal and relative, and we make no absolute frequency claim.
