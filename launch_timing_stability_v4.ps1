<# Run the frozen V4 single-thread Vivado infrastructure-stability gate. #>
param()
...
$gate = Join-Path $repo "timing_closure_gate_v4"
...
git diff --quiet -- .gitattributes launch_timing_stability_v4.ps1 timing_closure_gate_v4
if ($LASTEXITCODE -ne 0) {
    throw "the frozen V4 launcher/package has unstaged changes"
}
git diff --cached --quiet -- .gitattributes launch_timing_stability_v4.ps1 timing_closure_gate_v4
if ($LASTEXITCODE -ne 0) {
    throw "the frozen V4 launcher/package has staged changes"
}
if (Test-Path -LiteralPath $campaign) {
    throw "V4 stability campaign already exists and cannot be overwritten: $campaign"
}
if (Test-Path -LiteralPath $scratch) {
    throw "V4 scratch root already exists and cannot be overwritten: $scratch"
}
if (Get-Process -Name vivado -ErrorAction SilentlyContinue) {
    throw "another Vivado process is already running"
}
Invoke-LoggedPython -PythonArgs @("-m", "unittest", "timing_closure_gate_v4.test_stability")
