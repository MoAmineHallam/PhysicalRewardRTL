`timescale 1ns/1ps
// Logic Analyzer (FAST, shallow) for the silicon-Fmax sweep harness.
// Identical to la_axi_wide.v but with a SHALLOW buffer (DEPTH=2048) so the
// combinational readback mux (buffer[rd_index] -> RDATA) is ~16x smaller and
// therefore much faster.  The Phase-0 spike showed the 32768-deep distributed-
// RAM readback in la_axi_wide capped the capture harness at ~111 MHz, too low
// to measure 100-250 MHz accelerator designs.  We only ever score ~1024
// samples, so 2048 is ample, and the shallow mux lets the harness clock far
// higher than any design under test.
//
