...
class Attention(nn.Module):
    ...
    def forward(self, x, past=None, use_cache=False):
        ...
        with region(self.trace_regions, 'attention.sdpa'):
            if self.decode_backend == 'explicit' and offset and t == 1:
...
class Decoder(nn.Module):
    ...
    def set_decode_backend(self, backend):
        if backend not in ('sdpa', 'explicit'):
            raise ValueError('unknown decode attention backend')
        for block in self.blocks:
            block.attention.decode_backend = backend
